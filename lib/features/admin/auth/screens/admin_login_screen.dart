  ...
  Future<void> _login() async {
    ...
    try {
      ...
      if (!isAdmin) {
        ...
        if (mounted) {
          setState(() => _error = accessDeniedMsg);
          AppSnackbar.showWarning(
            context,
            message: accessDeniedMsg,
            error: 'User ${_emailCtrl.text.trim()} attempted admin login without admin claim.',
          );
        }
        return;
      }
      ...
      if (mounted) {
        AppSnackbar.showSuccess(context, 'Signed in successfully to Admin Portal');
        context.go('/admin/dashboard');
      }
    } catch (e, stack) {
      debugPrint('🚨 [Admin Login Failure]: $e');
      ...
      if (mounted) {
        ...
        AppSnackbar.showError(
          ...
          error: e,
          stackTrace: stack,
          fallbackMessage: 'Admin sign-in failed. Please check your credentials.',
        );
      }
    } finally {
              ...
              child: Column(
                ...
                children: [
                  Center(
                    child: Column(
                      children: [
                        const AppLogo(size: 68, borderRadius: 16),
